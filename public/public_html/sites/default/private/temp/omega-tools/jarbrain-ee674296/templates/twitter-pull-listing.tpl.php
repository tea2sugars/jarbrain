      ...
      <ul class="tweets-pulled-listing">
        <?php foreach ($tweets as $tweet_key => $tweet): ?>
          <li>
            ...
            <?php if (!empty($title)): ?>
              <h4><?php print $title; ?><span><span class="tweet-author">(@<?php print l($tweet->username, 'http://twitter.com/' . $tweet->username); ?>)</span></span><span class="tweet-date"><?php print l($tweet->time_ago, 'http://twitter.com/' . $tweet->username . '/status/' . $tweet->id); ?></span></h4>
            <?php endif; ?>
